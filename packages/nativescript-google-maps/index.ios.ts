...
export class MapView extends MapViewBase {
  ...
  animateCameraUpdate(update: CameraUpdate) {
      if (this.mapAnimationsEnabled)
        this.nativeView.animateWithCameraUpdate(update.ios)
      else
        this.nativeView.moveCamera(update.ios)
  }
...
export class CameraUpdate extends CameraUpdateBase {
    private _ios: any; /* GMSCameraUpdate */
    get ios() {
        return this._ios;
    }
    ...
    public static zoomIn(): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.zoomIn());
    }

    public static zoomOut(): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.zoomOut());
    }

    public static zoomBy(amount: number): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.zoomBy(amount));
    }

    public static zoomTo(level: number): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.zoomTo(level));
    }

    public static setTarget(target: Position): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.setTarget(target.ios));
    }

    public static setTargetWithZoom(target: Position, zoom: number): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.setTargetZoom(target.ios, zoom));
    }

    public static fitBounds(bounds: Bounds): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.fitBounds(bounds.ios));
    }

    public static fitBoundsWithPadding(bounds: Bounds, padding: number): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.fitBoundsWithPadding(bounds.ios, padding));
    }

    public static fitBoundsWithEdgeInsets(bounds: Bounds, insets: [number, number, number, number]): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.fitBoundsWithEdgeInsets(bounds.ios, UIEdgeInsetsMake(insets[0], insets[3], insets[2], insets[1])));
    }

    public static scrollByXY(x: number, y: number): CameraUpdate {
        return new CameraUpdate(GMSCameraUpdate.scrollByXY(x, y));
    }
}
...
export class Bounds extends BoundsBase {
  ...
  public static fromCoordinatesArray(coordinates: Position[]): Bounds {
      const path = GMSMutablePath.path();
      for (const coord of coordinates)
        path.addCoordinate(coord.ios)

      return new Bounds(GMSCoordinateBounds.new().initWithPath(path))
  }
}
...
export class Polyline extends PolylineBase {
  ...
  get visible(): boolean {
      return Boolean(this._ios.map);
  }

  set visible(value: boolean) {
      this._ios.map = value ? this._map : null;
  }
